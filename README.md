...
- タイトル・概要・日本時間の公開日時・タグ・公開設定を設定します。「記事設定」から要点、ファイル名、Markdownのダウンロードも操作できます。
- 入力中の内容はブラウザ内に自動退避されます。**「保存する」または `⌘S` / `Ctrl+S`** で `src/content/news/` に書き込みます。**「保存して公開」** は保存後に `pnpm run deploy:news` を実行し、`news.noguchy.me` へ反映します。初回だけ `pnpm exec wrangler login` が必要です。自動退避は同じブラウザ・同じURL内でのみ復元でき、ブラウザのデータを消すと失われます。
- 下書きはサイトに表示されません。「公開対象」で保存した記事は、**「保存して公開」** で公開できます。エディタは最後に成功したデプロイ時点のリビジョンを記録し、記事ごとに「公開済み」「未公開の変更」「公開待ち」を表示します。**保存だけではデプロイ・コミットは行いません。**
